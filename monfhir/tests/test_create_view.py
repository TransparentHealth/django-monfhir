...
class FHIR_Create_TestCase(TestCase):
    ...
    def setUp(self):
        ...
        # SupportedResourceType.objects.create(resource_type="Organization")
        USERNAME_FOR_TEST = b"alan"
        PASSWORD_FOR_TEST = b"p"
        self.credentials = b'%s:%s' % (USERNAME_FOR_TEST, PASSWORD_FOR_TEST)
        self.credentials = base64.b64encode(self.credentials)
        
            
        self.resource_type = "Practitioner"
        self.client=Client()
        self.client.defaults['HTTP_AUTHORIZATION'] = b'Basic ' + self.credentials
        #self.client.defaults['HTTP_X_REQUESTED_WITH'] ='XMLHttpRequest'
        
        print("RESOURCE_TYPE", self.resource_type)
        self.url=reverse('monfhir_create', args = (self.resource_type,))
    ...
    def test_create_fhir(self):
        ...
        response = c.post(self.url, json.dumps(j, indent=4),
